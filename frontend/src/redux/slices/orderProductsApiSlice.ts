      ...
      async onQueryStarted({ ...patch }, { dispatch, queryFulfilled }) {
        const patchResult = dispatch(
          ordersApiSlice.util.updateQueryData(
            ...
            undefined,
            (draftCart: Order) => {
              const itemToUpdate = draftCart.products.find(
                ({ orderProductId }) => {
      ...
      async onQueryStarted({ ...patch }, { dispatch, queryFulfilled }) {
        const patchResult = dispatch(
          ordersApiSlice.util.updateQueryData(
            ...
            undefined,
            (draftCart: Order) => {
              draftCart.products = draftCart.products.filter(
                ({ id }) => id !== patch.id,
              );
            },
          ),
...
export const { useUpdateOrderProductMutation, useDeleteOrderProductMutation } =
